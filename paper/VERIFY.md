...
**Paper**: 8 pages (7 of content + the references), 2 figures, 3 tables, 0 overfull boxes, 0
undefined references or citations, 8 `\todo`s. Builds clean. **Nothing left in it is a writing
task.** The eight split three ways: author input (title, group name, co-author list and their
affiliations, funding); blocked on data not on this machine (the study population — see §IV-A);
blocked on a paywall (re-checking the Unger numbers). The two repository footnotes resolve when the
repositories go public.

**Affiliation settled 2026-08-25**: Lake Lucerne Institute, Vitznau, Switzerland, with the
...
**Tables**: I trajectories (bias IQR as a fifth row); **II measures, now `table*` (full width) with
95% bootstrap intervals on all five r columns** — optical r_s/r_av, markerless r_s/r_av, 30 Hz r_s —
generated by `scripts/measure_cis.py` -> `table3_cis.csv`; III boundaries. Latency, capture-rate and
alternative-measure tables were cut for length; their scripts and CSVs remain.

**Bibliography**: 10 entries, **all ten cited** (checked 2026-08-25 — the closing todo had claimed
three were verified but uncited, which was stale), every field checked against a publisher record,
PDFs of 7 in `paper/refs/`. Provenance and the two metadata errors caught are at the end of
`refs.bib`.

**The Unger re-check is blocked, not merely undone** (established 2026-08-25). arXiv has only v1 of
2411.14992 — the API returns a single version entry, so there is no updated preprint to diff against.
Unpaywall reports `is_oa: false` for 10.1109/TMRB.2025.3605962 with no OA location. Every number
quoted from them therefore remains an arXiv v1 reading, and closing it needs a subscription, not more
searching. Do not spend time re-searching this.

**Headline numbers, all current**: 750 trials scored of 790 cached of 842 admitted; ten of twelve
measures at r_s >= 0.84 and r_av >= 0.93 end-to-end, eleven with optical windows, ten of twelve again
at 30 Hz; 16.8 s per trial for a 9.2 s recording, 1.3 s of it work that cannot begin before the last
frame; 4.9 % segmenter declines, **12.9 % at the four three-camera units against 0.4 % elsewhere**.

**PAGE BUDGET: deferred, not broken** (2026-08-25). Whether 7 content pages is an overrun depends
on the venue, which is not fixed — ICORR would be 6, the TMRB track where~\cite{unger2024} landed
would not. Measured: deleting Fig. 1 restores 6 exactly and nothing else does, not shrinking it and
not any float placement. **Fig. 1 has NOT been deleted**; the author's decision is to keep 7 and pick
a venue first.

### If you read one thing before editing

**Roughly thirty claims in this paper turned out to be wrong over two days, and they failed in four
recurring ways.** The lesson is not any single number; it is the pattern.

1. **A plausible mechanism asserted instead of measured.** Four of six phase-boundary offset "causes"
   were wrong; "HOLD is the second axis" was wrong; "position beats velocity at the settle" was
   wrong. Every one was cheap to test and I reported it before testing.
2. **A statistic read past its resolution.** "17 vs 33 ms" is one frame at 60 Hz, and the same rule
   reads both depending on the subset. Five claims sat inside their own uncertainty until an interval
   was computed: the boundary medians, interjoint's window "gain", interjoint's 30 Hz "collapse",
   "none degrades by more than 0.04", "three move by more than 0.01". **Compute the interval before
   interpreting a difference** — Table II now carries them for this reason.
3. **A number quoted from the wrong artifact.** Interjoint's variance concentration came from the
   pre-anat12 file; the Unger figures come from the arXiv version, not the published one (STILL OPEN);
   the calibration file ranks units over cameras the pipeline discards.
4. **A property claimed without checking every constant in the call path.** §III-C's
   "no absolute thresholds" was wrong three times running, each time because another constant sat one
   level down — `LEAVE_REST` 30 mm, `ARRIVE_REST` 40 mm, then `GRASP_FLAT_MMPS` 40 mm/s.

**Corollary worth keeping:** boundary-agreement metrics do not predict measure quality. A velocity
reach onset was better on *every* boundary statistic and destroyed interjoint coordination
(0.571 -> 0.333). Re-score before trusting a boundary change.

### Open

- **Fig. 1 / page budget** — the only lever that reaches 6 pages.
- **`unger2024` numbers** — read off arXiv v1; the published IEEE version is paywalled. Accepted as a
  known risk by decision, but re-check if access appears.
- **Study population** — FMA-UE totals and the Record ID -> P-label key are on neither this machine nor
  the mounted share. Ethics statement still deferred to `unger2024`.
- **Front matter** — title, authors, affiliations, repository URL, funding.
- **Not taken, deliberately**: limits of agreement at the measure level; movement units under a
  changed settle rule (the one measure the attenuation argument cannot protect).
...
### Both READMEs rewritten 2026-08-25, and what was wrong with them

The **top-level README** was still the pre-paper one. It pointed "Run it" at
`python -m pipeline.pipeline`, which the reorg moved to `pipeline/_archive_20260820/`; its module
table named `pipeline.cup_detect`, `pose_keypoints`, `flow_speed` and `speed_blend`, none of which
are in `pipeline/` any more; and it led with headline results from **n = 12 on two participants**,
superseded by 750 trials on eleven units. Rewritten around the paper's numbers, with the three
`OT_SEG_*` variables stated as required rather than optional.

**`paper/README.md` carried four stale claims**, all now fixed:
- "Cohort: 11 participants" — it is **ten participants in eleven recording units**. This is exactly
  the distinction §III-A makes, and the README was seeding the error it warns about.
- unger2024 as "(ICORR 2025, arXiv:2411.14992)" — published in T-MRB since.
- "`main.tex` uses these" of **both** correlation panels. It inputs only `fig4b_mmcseg_anat12.png`.
  `fig4a` is generated as its pair and feeds `fig4_pair.csv`, but is not in the paper.
- **two reproduce chains that had drifted apart** — the older one under "## Reproduce" was missing
  `trunk_theta.py` and `measure_cis.py`, so following it would have rebuilt the paper without the
  sternum offset or the intervals. The chain is now stated once.

**Reproducibility check, 2026-08-25**: `make_tables_tex.py` re-run from the committed CSVs emits
`table1_trajectories.tex` and `table3_measures.tex` **byte-identical** to what is committed. The
last mile of the chain reproduces.

Its diagnostic print said "11 participants, 224 trials". Both were wrong labels rather than wrong
data: `part` counts recording units, and `trial` is a per-unit index reused across units, so its
`nunique()` is a label count. Now prints "11 recording units, 790 trials", which is what the paper
says. Fixed rather than left, because this is where the README's "11 participants" came from.

### [!] `pipeline/` IS NOT THE PAPER'S PIPELINE (established 2026-08-25)

Anyone extracting a standalone repository must read this first. The eight modules in `pipeline/` are
**not** sufficient, and two of them are superseded versions of what the paper actually ran:

| stage | what `pipeline/` has | what the paper used |
|---|---|---|
| segmentation | `pipeline/segment.py` — the cup-only van Andel base segmenter | `scripts/seg_sequential.py::segment_sequential`, a different algorithm |
| measures | `pipeline/score.py` — position-derived only; its docstring says the five angle measures are **deliberately not ported** | those seven **plus** `scripts/score_vs_automq.py::angle_measures_automq`, `_planar_body_angles`, `_elbow_series` |
| per-frame front end | absent | `pipeline/_archive_20260820/{cup_detect,pose_keypoints}.py` |

So the paper's method spans `pipeline/`, `scripts/`, and an `_archive_` directory. `pipeline/score.py`
IS used, but only for its half; reading its docstring alone would tell you the paper's angle measures
do not exist.

**The good news, measured rather than assumed:** the two algorithms that live in `scripts/` are
already pure functions on arrays. `segment_sequential(cup_xyz, hand_xyz, mouth_xyz, fps=...)` takes
no cohort object, and `angle_measures_automq(pose, ph, side)` takes a pose dict and a phase dict. The
DELTA coupling in both files (`compare_pose_omc_delta`, `results_v3_delta`, `gnn_train`) is
module-level import for the **validation `main()`**, not for the algorithm. An extraction is a move
plus an import cut, not a rewrite.

**The bar for that extraction:** it must reproduce the shipped boundaries and measures on the cached
DELTA trials bit-for-bit, and the test that shows it must ship with it. An unverified re-port is
