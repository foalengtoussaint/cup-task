...
the same 2D keypoints, so they are directly comparable against OMC on the same frames.
    ...
    cloud   pipeline.cloud_velocity: Shi-Tomasi sub-features in a ROI, epipolar NCC matching,
            triangulate, Kabsch/RANSAC between consecutive clouds. Re-detects every frame.
    flow    pipeline.flow_speed (SHIPPING): PyrLK at the single keypoint, lift to 3D via the
...
def main(argv=None):
    ...
    from pipeline import flow_speed as FS
