#!/usr/bin/env bash
# Rebuild the pose caches with the RELAXED reprojection gate (pipeline/triangulate.py, 2026-08) so the
...
#         (4) score_vs_automq (re-score with planar default) -> out/scoring/score_vs_automq.csv
set -e
...
python -u scripts/score_vs_automq.py --out out/scoring/score_vs_automq.csv --parts $PARTS
