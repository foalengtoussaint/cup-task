...
WINDOWS = [0, 6, 4, 3, 2, 1]               # extra LOW-PASS cutoff Hz; 0 = none
...
def _interjoint(flex, elb, ph, w):
    ...
    if not reach or reach[1] - reach[0] < 12:
        return np.nan
    if w and w > 0 and len(flex) > 15:
        b, a = butter(2, w / (0.5 * FPS), btype="low")
        flex = filtfilt(b, a, np.nan_to_num(flex, nan=np.nanmedian(flex)))
        elb = filtfilt(b, a, np.nan_to_num(elb, nan=np.nanmedian(elb)))
...
def main():
    ...
    df = pd.DataFrame(rows); df.to_csv(ROOT / "out/scoring/interjoint_lowpass_sweep.csv", index=False)
    ...
    for w in WINDOWS:
        ...
        lbl = "none (6Hz)" if w == 0 else f"{w} Hz"
        print(f"{lbl:>20}{rs:>14.2f}{err:>8.3f}{p5:>9.3f}{len(g):>6}")
    print("\nwrote out/scoring/interjoint_lowpass_sweep.csv\nDONE", flush=True)
