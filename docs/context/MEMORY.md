...
- [Multi-view 3D lifter plan](project_mv3d_lifter_plan.md) — YOLO CSPDarknet neck → learned triangulation head; pretrain BEDLAM(majority,gated)+AIST++(gated)+Panoptic(downloading) w/ random 3-6 view sampling, finetune 12 DELTA trials; SINGLE-PERSON only; must beat triangulation+SmoothNet on apex/jitter not position; Monday unlock: HF login + AIST++ + OneDrive weights
