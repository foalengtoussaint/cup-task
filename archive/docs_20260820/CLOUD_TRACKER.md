...
| Component | What / why |
|---|---|
| **Seed shape doesn't matter** | A generic 20 mm blob beats the true 40×95 cylinder (12.8 vs 15.6 mm/s). It only has to land points *on* the object; too big and seeds fly off (80×160 → 19 % coverage). This is why the tracker is object-agnostic and runs on a bare keypoint. **Cohort update (2026-07-23, n=12): fill the VOLUME, don't trace the surface** — a filled ball r=40 at n=48 (radius ∝ ∛U) keeps the median (13.2 vs 13.6, tie) but fixes the tail (worst trial 27.2→15.7) and coverage (52→67%; 94% with the gate off), because face-projected seeds land on 2D texture instead of the aperture-limited silhouette, keeping inliers above the min_inliers floor exactly at fast frames. Not yet the class default. |
...
| **Rigidity gate** (`max_scale_dev=0.01`) | Fit a *similarity* (Umeyama) too; scale ≠ 1 means the cloud deformed. **⚠ RETIRED as an error signal (2026-07-23, n=12):** its ρ +0.36 with error is *entirely* speed in disguise — within speed bins ρ(|s−1|, err) ≈ 0 (+0.06/+0.01/−0.01/+0.05), and gate-off is more accurate in **every** speed bin (fast bin 48.9→37.7) at 94% vs 67% coverage. The gate's per-trial "wins" were composition (it refuses the hard fast frames, flattering its median — Simpson's). It helped the original thin rim-24 seed only because refusing fast frames helped an evidence-starved cloud. With a filled-volume n=48 seed, run `max_scale_dev=None`. See WORKLOG 2026-07-23. |
