...
def track_cup_3d_from_cache(cache_json: str | Path, calib: dict,
                            refine_ba: bool = True) -> list[dict]:
    """Build the 3D cup track from a cached per-camera dump + greedy consensus, then BUNDLE-ADJUST.
    ...
    refine_ba: after consensus has chosen WHICH cameras and WHICH frames, refine WHERE the point is
    with a distortion-aware Huber BA over exactly that kept set (cup_ba_refine), so every 3D point in
    the pipeline -- pose and cup alike -- is reconstructed the same way. It cannot change the gating:
    no frame is added, no rejected camera re-admitted. Running BA over ALL cameras instead measurably
    HURTS (drink-offset p90 20 -> 47 frames). False reproduces the pre-2026-08-20 linear-DLT cup.
    """
    ...
    for f in range(n):
        ...
        track.append({
            ...
            "n_cams": len(kept),
            "kept": sorted(kept),
            "uv": {c: obs[c] for c in kept if c in obs},   # kept-camera 2D, for the BA refine
        })
    if refine_ba:
        from pipeline import cup_ba_refine
        track = cup_ba_refine.refine_track(track, calib)
    return track
