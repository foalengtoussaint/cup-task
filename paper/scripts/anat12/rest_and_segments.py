...
def one(t):
    ...
    return dict(part=p, arm=("affected" if rec.get("condition")=="affected" else "unaffected"),
                ...
                ua_mmc_other=seglen(pose, other), ua_omc_other=seglen(po, other))
...
if __name__ == "__main__":
    ...
    g = df.groupby(["part","arm"]).agg(n=("rest_mmc","size"), rest_MMC=("rest_mmc","median"),
        rest_OMC=("rest_omc","median"), upperarm_MMC=("ua_mmc","median"),
        upperarm_OMC=("ua_omc","median")).round(1)
    g["rest_DIFF"] = (g.rest_MMC - g.rest_OMC).round(1)
    g["ua_DIFF"] = (g.upperarm_MMC - g.upperarm_OMC).round(1)
    print(g.to_string())
    print("\nL-vs-R upper-arm asymmetry WITHIN each modality (per participant):")
    ...
    df.to_csv(ROOT/"out/scoring/rest_and_segments.csv", index=False)
    print("\nwrote out/scoring/rest_and_segments.csv")
