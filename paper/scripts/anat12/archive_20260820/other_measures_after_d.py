...
def other_measures(recs, d, frame="trunk"):
    """Measures that a constant shoulder offset CAN change: interjoint (corr flex,elbow over
    Reaching) and peak elbow angular velocity (max |d elbow|*fps over Reaching).
    NOT computed: wrist-based measures (PV, times, movement units, total time) and trunk
    displacement -- all analytically INVARIANT to a constant shoulder shift."""
    rows=[]
    for r in recs:
        f, a, e = angles_with_d(r, d, frame)
        fm, am, em = r["mmc"]
        nr = min(r["n_reach"], len(f), len(fm))
        if nr < 12: continue
        rows.append(dict(part=r["part"], arm=r["arm"],
            ij_omc=_corr(f[:nr], e[:nr]), ij_mmc=_corr(fm[:nr], em[:nr]),
            pav_omc=float(np.nanmax(np.abs(np.diff(e[:nr])))*FPS),
            pav_mmc=float(np.nanmax(np.abs(np.diff(em[:nr])))*FPS)))
...
if __name__ == "__main__":
    ...
    for (p,a), recs in sorted(groups.items()):
        raw.append(other_measures(recs, np.zeros(3)))
        d = least_squares(resid, x0=np.zeros(3), args=(recs,), method="lm", max_nfev=200).x
        corr_.append(other_measures(recs, d))
        print(f"  {p} {a} done", flush=True)
    raw = pd.concat(raw); corr_ = pd.concat(corr_)
    print(f"\n{'measure':28}{'set':12}{'r_s':>8}{'med OMC':>10}{'med MMC':>10}   n")
    for nm, co, cm in (("interjoint coordination","ij_omc","ij_mmc"),
                       ("peak elbow ang vel [d/s]","pav_omc","pav_mmc")):
        for lab, d_ in (("raw", raw), ("corrected", corr_)):
            g = d_.dropna(subset=[co,cm])
            print(f"{nm:28}{lab:12}{spearmanr(g[co],g[cm]).correlation:>8.3f}"
                  f"{g[co].median():>10.2f}{g[cm].median():>10.2f}   {len(g)}")
    print("\nINVARIANT to a constant shoulder offset (not recomputed): peak velocity, time-to-peak,")
    print("time-to-first-peak, movement units, total movement time, trunk displacement.")
    raw.to_csv(ROOT/"out/scoring/other_measures_after_d.csv", index=False)
