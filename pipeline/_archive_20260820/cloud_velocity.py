...
⚠⚠ THIS MODULE'S PIPELINE DOES NOT WORK ON REAL DATA. USE `pipeline.cloud_track` INSTEAD.
...
`pipeline.cloud_track`, which reaches 29.7 mm/s on the same data.

    from pipeline.cloud_velocity import CloudVelocityTracker
...
class CloudVelocityTracker:
    ...
    (PyrLK) instead of re-detecting them -- see `pipeline.flow_speed`, which measures velocity
