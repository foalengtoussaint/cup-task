...
undefined references or citations, 10 `\todo`s. Builds clean. Every section is written except what
needs information not on this machine — title, authors/affiliations, repository URL, the study
population, funding.
...
**Bibliography**: 10 entries, every field checked against a publisher record, PDFs of 7 in
`paper/refs/`. Provenance and the two metadata errors caught are at the end of `refs.bib`.
...
**PAGE BUDGET IS BROKEN**: target 6 content pages, actual 7. Measured: deleting Fig. 1 restores 6
exactly and nothing else does — not shrinking it, not any float placement. Awaiting a decision.
